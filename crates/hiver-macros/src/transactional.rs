...
pub(crate) fn transactional_impl(attr: &TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #fn_vis #fn_async fn #fn_name(#fn_inputs) #fn_output {
            ...
            let tx_mgr = ::hiver_tx::global_tx_manager()
                .unwrap_or_else(|| ::std::sync::Arc::new(::hiver_tx::NoopTransactionManager));
            ...
            ::hiver_tx::synchronization::with_transaction_scope(async move {
                // Begin transaction
                // 开始事务
