...
Hiver 框架提供了结构化日志输出格式，包括启动横幅、彩色日志输出和启动信息记录。

## 功能特性 / Features

- ✅ **启动横幅 (Banner)** - HIVER ASCII 艺术
...
```rust
use hiver_observability::log::{Logger, LoggerConfig, LogLevel, LogFormat};

let config = LoggerConfig {
    level: LogLevel::Debug,
    format: LogFormat::Pretty,  // Hiver 格式
    with_thread: true,
    with_target: true,
    ..Default::default()
};
...
2026-01-24 19:35:25.785 |INFO| 10500 [main             ] n                                : Starting Hiver application
...
```rust
use hiver_observability::Banner;
...
```
 ...
 |  _  | (_) \__ \ |_  | |_| | | | | | |
 |_| |_|\___/|___/\__|  \___/|_| |_| |_|
Hiver v0.1.0-alpha | port: 8080 | profile: active
```

## 启动信息 / Startup Information

```rust
use hiver_observability::StartupLogger;
...
export HIVER_LOG_FORMAT=pretty
...
```rust
use hiver_observability::hiver_format::HiverFormatter;

let formatter = HiverFormatter::new()
    .with_colors(true)
    .with_app_name("MyApp")
...
```rust
use hiver_observability::log::{LoggerConfig, LogLevel, LogFormat, LogRotation};
...
1. **功能标志**: 需要启用 `hiver-format` 功能才能使用 Hiver 格式化
2. **颜色支持**: 自动检测终端是否支持颜色
