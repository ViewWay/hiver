...
//!
//! ```rust,no_run,ignore
//! use nexus_data_orm::sqlx::{SqlxQuery, SqlxPoolAdapter};
//!
//! // Build a query
...
pub trait FromRow: Sized {
    ...
    /// 从 Nexus RDBC 行构造此类型。
    fn from_rdbc_row(row: &nexus_data_rdbc::Row) -> Result<Self>;
}
...
impl VerifiedQuery {
    ...
    /// 执行此查询并获取所有行。
    pub async fn fetch_all<C: DatabaseClient>(&self, client: &C) -> Result<Vec<nexus_data_rdbc::Row>> {
        client
            .fetch_all(&self.sql)
            .await
