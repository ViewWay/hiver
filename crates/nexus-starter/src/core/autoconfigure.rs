...
//!
//! ```rust,ignore
//! use nexus_starter::core::autoconfigure::*;
//!
...
/// - `factory`: 工厂函数，负责在 `ApplicationContext` 中注册 Bean / Factory function that registers beans
...
/// Manages all registered auto-configuration entries, supports condition evaluation and priority sorting.
///
...
impl AutoConfigurationRegistry {
    ...
    /// # 参数 / Parameters
    ///
    /// - `entry`: 自动配置条目（应已通过 `with_condition` 设置条件）
    ///   / Auto-configuration entry (should have condition set via `with_condition`)
...
///
/// ```rust,ignore
/// use nexus_starter::core::autoconfigure::EnableAutoConfiguration;
///
/// // 此标记通常由框架内部使用
