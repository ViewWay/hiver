...
//! # Overview / 概述
//!
//! `nexus-shell` provides an interactive CLI REPL (Read-Eval-Print Loop) framework,
//! equivalent to Spring Shell in the Spring ecosystem.
//!
//! `nexus-shell` 提供交互式CLI REPL（读取-求值-打印循环）框架，
//! `等价于Spring生态系统中的Spring` Shell。
//!
//! # Features / 功能
//!
//! - **REPL Loop** — Interactive command-line with history and tab completion
//!   **REPL循环** — 带历史记录和Tab补全的交互式命令行
//! - **Command Registry** — Dynamic command registration with trait-based dispatch
//!   **命令注册表** — 基于trait的动态命令注册
//! - **Built-in Commands** — help, clear, exit, stacktrace, script, echo, history
//!   **内置命令** — help、clear、exit、stacktrace、script、echo、history
//! - **Custom Prompts** — Colored, configurable prompts
//!   **自定义提示符** — 彩色、可配置的提示符
//! - **Input Validation** — Command input validation and sanitization
//!   **输入验证** — 命令输入验证和清理
//! - **Multiple Output Formats** — Plain text, JSON, Table
//!   **多种输出格式** — 纯文本、JSON、表格
...
//!
//! ```rust,no_run,ignore
//! use nexus_shell::{ShellBuilder, ShellConfig};
//! use nexus_shell::command::{Command, CommandMeta};
//! use nexus_shell::result::ShellResult;
//!
...
//! fn main() -> nexus_shell::result::ShellResult<()> {
...
//!
//! ```rust,no_run,ignore
//! use nexus_shell_macros::shell_component;
//! use nexus_shell::command::{Command, CommandMeta};
//! use nexus_shell::result::ShellResult;
//!
//! #[shell_component]
