...
impl ApiDocPdf {
    ...
    pub fn to_html(&self) -> String {
        ...
        for section in &self.sections {
            ...
            for endpoint in &section.endpoints {
                ...
                if !endpoint.parameters.is_empty() {
                    ...
                    html.push_str("        <thead><tr><th>Name</th><th>In</th><th>Required</th><th>Type</th><th>Description</th></tr></thead>\n");
                    html.push_str("        <tbody>\n");
                    for p in &endpoint.parameters {
        ...
        let _ = writeln!(html, "    <p>Generated by Hiver OpenAPI | API Documentation v{}</p>", escape_html(&self.version));
        html.push_str("  </div>\n");
...
mod tests {
    use super::*;
    use crate::{OpenApiBuilder, Operation, Parameter, PathItem, RequestBody, Response};
    ...
    fn test_html_method_colors() {
        let openapi = OpenApiBuilder::new()
            .title("Color Test")
            .version("1.0.0")
            .add_path("/items/{id}", PathItem::new()
