...
//! # How it works / 工作原理
//!
//! 1. Entity carries a version number (typically starting at 0).
//!    实体携带一个版本号（通常从 0 开始）。
//! 2. On read, the version is captured.
//!    读取时捕获版本号。
//! 3. On update, the version is checked: if it has changed since read,
//!    the update is rejected with `OptimisticLockError`.
//!    更新时检查版本号：如果自读取以来版本号已改变，
//!    更新将被拒绝并返回 `OptimisticLockError`。
...
///
/// ```rust
/// use nexus_data_commons::optimistic_lock::OptimisticLockError;
///
...
///
/// ```rust
/// use nexus_data_commons::optimistic_lock::Version;
///
/// #[derive(Debug, Clone)]
...
///
/// ```rust
/// use nexus_data_commons::optimistic_lock::{Versioned, Version, OptimisticLockError};
///
/// #[derive(Debug, Clone)]
/// struct User {
///     id: i32,
...
///     fn version(&self) -> i64 { self.ver }
///     fn set_version(&mut self, v: i64) { self.ver = v; }
/// }
///
/// let user = User { id: 1, name: "Alice".into(), ver: 3 };
...
/// versioned.verify_version("User", "1", current_db_version).unwrap();
/// ```
#[derive(Debug, Clone)]
...
///
/// ```rust,ignore
/// use nexus_data_commons::optimistic_lock::VersionCheckedUpdate;
///
