...
/// `将GraphQL错误转换为HTTP响应`
pub fn errors_to_response(errors: &GraphQLErrors) -> nexus_http::Response {
    let body = serde_json::json!({
        "errors": errors.0,
    });
    nexus_http::Response::json(&body)
}

/// Convert a single GraphQL error to an HTTP response
/// `将单个GraphQL错误转换为HTTP响应`
pub fn error_to_response(error: &GraphQLError) -> nexus_http::Response {
