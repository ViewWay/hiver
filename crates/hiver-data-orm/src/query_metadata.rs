...
use hiver_data_rdbc::{Error as R2dbcError, QueryParam, Result as R2dbcResult};
        ...
        while let Some(c) = chars.next()
        {
            if c == '$'
            {
                ...
                if !cur.is_empty()
                {
                    ...
                    if params.len() <= idx
                    {
                        params.resize(idx + 1, format!("param{}", idx + 1));
                    }
                    cur.clear();
                }
            }
            ...
            ParamStyle::Named =>
            {
                ...
                for name in &self.param_names
                {
                    ...
                    if let Some(pos) = sql[off..].find(&ph)
                    {
                        ...
                        values.push(params.get(name).cloned().map(QueryParam::from).ok_or_else(
                            || R2dbcError::sql(format!("Missing parameter: {}", name)),
                        )?);
                        idx += 1;
                        off += pos + rep.len();
                    }
...
mod tests
{
    ...
    {
        assert_eq!(
            QueryMetadata::new("SELECT 1", ParamStyle::Named).query_type(),
            QueryType::Select
        );
    }
    #[test]
    ...
    {
        assert_eq!(
            QueryMetadata::new("INSERT INTO t VALUES(:v)", ParamStyle::Named).query_type(),
            QueryType::Insert
        );
    }
    #[test]
    ...
    {
        assert_eq!(
            QueryMetadata::new("UPDATE t SET a=1", ParamStyle::Named).query_type(),
            QueryType::Update
        );
    }
    #[test]
    ...
    {
        assert_eq!(
            QueryMetadata::new("DELETE FROM t", ParamStyle::Named).query_type(),
            QueryType::Delete
        );
    }
    #[test]
    ...
    {
        assert_eq!(QueryMetadata::new("WHERE a=:x AND b=:y", ParamStyle::Named).param_names(), &[
            "x", "y"
        ]);
    }
    #[test]
    fn test_mybatis_params()
    {
    ...
    fn test_bind_named()
    {
        ...
        p.insert("id".into(), serde_json::json!(1));
        p.insert("name".into(), serde_json::json!("A"));
