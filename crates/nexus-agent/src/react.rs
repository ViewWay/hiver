...
use nexus_ai::prompt::PromptTemplate;
use nexus_ai::chat_model::{ChatMessage, ChatModel, ChatRequest};
use nexus_ai::tool::{ToolCall, ToolExecutor, ToolRegistry};
...
IMPORTANT: Only use tools from the available tools list. If no tool is appropriate, \
...
///
/// ```rust,ignore
/// use nexus_agent::react::{ReActAgent, ReActConfig};
...
mod tests {
    ...
    fn create_test_agent() -> ReActAgent {
        ...
        impl ChatModel for MockModel {
            ...
            ) -> Result<nexus_ai::chat_model::ChatResponse, ModelError> {
                Ok(nexus_ai::chat_model::ChatResponse::new("mock", "mock"))
            }

            async fn stream(
                &self,
                _request: ChatRequest,
            ) -> Result<nexus_ai::chat_model::ChatStream, ModelError> {
