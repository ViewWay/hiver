...
mod tests {
    ...
    async fn test_not_filter() {
        let inner =
            Arc::new(PredicateFilter::new(|msg| msg.get_payload::<i32>().is_some_and(|v| v > 10)));
    ...
    async fn test_and_filter() {
        let filter = AndFilter::new(vec![
            ...
            Arc::new(PredicateFilter::new(|msg| msg.get_payload::<i32>().is_some_and(|v| v < 100))),
        ]);
    ...
    async fn test_or_filter() {
        let filter = OrFilter::new(vec![
            Arc::new(PredicateFilter::new(|msg| {
                msg.get_payload::<String>().is_some_and(|s| s == "special")
            })),
            Arc::new(PredicateFilter::new(|msg| msg.get_payload::<i32>().is_some_and(|v| v > 100))),
        ]);
