...
//!
//! ```rust,no_run,ignore
//! use nexus_data_orm::repository::OrmRepository;
//! use std::sync::Arc;
//!
...
use nexus_data_commons::{CrudRepository, Repository};
...
impl<T: Model + Send + Sync> OrmRepository<T> {
    ...
    ///
    /// ```rust,no_run,ignore
    /// let template = nexus_tx::TransactionTemplate::new(mgr);
...
fn to_commons<T: Model>(e: crate::Error) -> nexus_data_commons::Error {
    if e.is_not_found() {
        nexus_data_commons::Error::entity_not_found(T::table_name(), "")
    } else if e.is_validation() {
        nexus_data_commons::Error::data_integrity_violation(e.to_string())
    } else {
        nexus_data_commons::Error::connection(e.to_string())
    }
}
    ...
    T: Model + Send + Sync + serde::de::DeserializeOwned + serde::Serialize + 'static,
{
    ...
    async fn save(&self, entity: T) -> std::result::Result<T, Self::Error> {
        ...
        if pk.is_empty() || pk == "0" {
            ...
            let json = serde_json::to_value(&entity)
                .map_err(|e| nexus_data_commons::Error::data_integrity_violation(e.to_string()))?;
            let map = match &json {
                serde_json::Value::Object(m) => m,
                _ => return Err(nexus_data_commons::Error::data_integrity_violation("not an object")),
            };
            ...
            match self.client.fetch_one_params(&sql, &params).await {
                Ok(Some(row)) => row
                    .deserialize()
                    .map_err(|e| nexus_data_commons::Error::data_integrity_violation(e.to_string())),
                Ok(None) => Err(nexus_data_commons::Error::entity_not_found(T::table_name(), "INSERT returned no row")),
                Err(e) => Err(nexus_data_commons::Error::connection(e.to_string())),
            }
        } else {
            // Update
            let json = serde_json::to_value(&entity)
            ...
            match self.client.fetch_one_params(&sql, &params).await {
                Ok(Some(row)) => row
                    .deserialize()
                    .map_err(|e| nexus_data_commons::Error::data_integrity_violation(e.to_string())),
                Ok(None) => Err(nexus_data_commons::Error::entity_not_found(T::table_name(), &pk_param)),
                Err(e) => Err(nexus_data_commons::Error::connection(e.to_string())),
            }
        }
    }
    ...
    async fn find_by_id(&self, id: String) -> std::result::Result<Option<T>, Self::Error> {
        ...
        let row = self
            ...
            .await
            .map_err(|e| nexus_data_commons::Error::connection(e.to_string()))?;
        match row {
            Some(r) => r
                .deserialize()
                .map(Some)
...
fn collect_rows<T: serde::de::DeserializeOwned>(rows: Vec<nexus_data_rdbc::Row>) -> Result<Vec<T>> {
