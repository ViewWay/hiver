...
//!
//! ```rust,ignore
//! use hiver_events::transactional_listener::*;
//! use hiver_events::event::ApplicationEvent;
//!
...
/// 事务生命周期与事件发布之间的桥接器
///
/// This struct hooks into a transaction lifecycle (when available via `hiver-tx`)
/// and automatically triggers the appropriate transactional event phase callbacks.
///
/// 此结构体挂钩到事务生命周期（当通过 `hiver-tx` 可用时），并自动触发相应的事务事件阶段回调。
///
/// # Spring Equivalent / Spring等价物
///
/// ```java
