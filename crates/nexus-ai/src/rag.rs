...
use crate::chat_model::{ChatModel, ChatRequest, ChatStream, ModelError};
...
impl Default for RagConfig {
    fn default() -> Self {
        Self {
            ...
            chunk_size: 500,
            chunk_overlap: 50,
            system_prompt: "You are a helpful assistant. Answer the user's question based on the provided context. If the context does not contain enough information, say so.".to_string(),
        }
    }
}
...
///
/// ```rust,ignore
/// use nexus_ai::rag::{RagPipeline, RagConfig};
/// use std::sync::Arc;
///
