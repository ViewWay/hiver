...
fn extract_condition_fn(
    ...
    let cond_fn = format_ident!("__nexus_condition_{}", input.ident);
        ...
        if attr.path().is_ident("conditional_on_property") {
            ...
            if !key.is_empty() {
                if let Some(ref v) = value {
                    let def = quote! {
                        fn #cond_fn(ctx: &::nexus_starter::core::ApplicationContext) -> bool {
    ...
    (
        quote! {},
        quote! { ::nexus_starter::core::registry::always_true },
    )
}
...
pub fn generate_bean_registration(
    ...
    let struct_name = &input.ident;
    let factory_fn = format_ident!("__nexus_factory_{}", struct_name);
    ...
    quote! {
        fn #factory_fn(ctx: &::nexus_starter::core::ApplicationContext) -> ::std::boxed::Box<dyn ::std::any::Any + ::std::marker::Send + ::std::marker::Sync> {
        ...
        ::inventory::submit! {
            ::nexus_starter::core::registry::BeanDescriptor {
