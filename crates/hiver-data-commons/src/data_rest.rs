...
pub fn generate_crud_endpoints(config: &RestResourceConfig) -> Vec<RestEndpoint>
{
    ...
    vec![
        RestEndpoint {
            ...
            path: base.clone(),
            description: format!("List {}", config.resource_name),
        },
        RestEndpoint {
            method: RestMethod::Get,
            path: format!("{}/{{id}}", base),
            description: format!("Get {} by ID", config.resource_name),
        },
        RestEndpoint {
            method: RestMethod::Post,
            path: base.clone(),
            description: format!("Create {}", config.resource_name),
        },
        RestEndpoint {
            method: RestMethod::Put,
            path: format!("{}/{{id}}", base),
            description: format!("Update {} by ID", config.resource_name),
        },
        RestEndpoint {
            method: RestMethod::Delete,
            path: format!("{}/{{id}}", base),
            description: format!("Delete {} by ID", config.resource_name),
        },
    ]
}
