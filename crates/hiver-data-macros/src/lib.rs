...
//!
//! ```rust,no_run,ignore
//! use hiver_data_macros::{Model, Repository};
//! use hiver_data_commons::CrudRepository;
//!
...
fn model_derive_impl(input: DeriveInput) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl #vis hiver_data_orm::Model for #struct_name {
            fn meta() -> hiver_data_orm::ModelMeta {
                hiver_data_orm::ModelMeta::new(#table_name)
                    #(.add_column(#column_definitions))*
            }

            fn primary_key(&self) -> hiver_data_orm::Result<String> {
                Ok(format!("{}", self.#pk_field))
            }

            fn set_primary_key(&mut self, value: String) -> hiver_data_orm::Result<()> {
                self.#pk_field = value.parse().map_err(|e| {
                    hiver_data_orm::Error::validation(format!("Invalid primary key: {}", e))
                })?;
                Ok(())
            }
            ...
            /// 从数据库行构造模型实例。
            fn from_row(row: &hiver_data_rdbc::Row) -> hiver_data_orm::Result<Self> {
                Ok(Self {
                    #(#from_row_fields),*
                })
            }
...
fn map_type_to_column_type(ty: &Type) -> proc_macro2::TokenStream {
    ...
    match simplified.as_str() {
        ...
        "i64" => quote!(hiver_data_orm::ColumnType::I64),
        "i128" => quote!(hiver_data_orm::ColumnType::I128),
        "u8" => quote!(hiver_data_orm::ColumnType::U8),
        "u16" => quote!(hiver_data_orm::ColumnType::U16),
        "u32" => quote!(hiver_data_orm::ColumnType::U32),
        "u64" => quote!(hiver_data_orm::ColumnType::U64),
        "f32" => quote!(hiver_data_orm::ColumnType::F32),
        "f64" => quote!(hiver_data_orm::ColumnType::F64),
        "bool" => quote!(hiver_data_orm::ColumnType::Bool),
        "String" | "string" => quote!(hiver_data_orm::ColumnType::String),
        "str" => quote!(hiver_data_orm::ColumnType::String),
        "Vec<u8>" => quote!(hiver_data_orm::ColumnType::Bytes),
        "Uuid" | "uuid" => quote!(hiver_data_orm::ColumnType::Uuid),
        "DateTime" | "chrono::DateTime" => quote!(hiver_data_orm::ColumnType::Timestamp),
        "Date" | "chrono::Date" => quote!(hiver_data_orm::ColumnType::Date),
        "serde_json::Value" => quote!(hiver_data_orm::ColumnType::Json),
        _ => quote!(hiver_data_orm::ColumnType::Text),
    }
}
...
///
/// ```rust,no_run,ignore
/// use hiver_data_macros::Repository;
/// use hiver_data_commons::{CrudRepository, Result, Error};
///
/// #[Repository]
/// pub trait UserRepository: CrudRepository<User, i64> {
...
fn repository_impl(trait_def: syn::ItemTrait) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #vis struct #struct_name {
            client: std::sync::Arc<hiver_data_rdbc::SqlxPoolClient>,
            table: &'static str,
        }

        impl #struct_name {
            /// Create a new repository instance
            pub fn new(client: std::sync::Arc<hiver_data_rdbc::SqlxPoolClient>) -> Self {
                Self {
                    client,
                    table: #table_name,
                }
            ...
            pub fn client(&self) -> &std::sync::Arc<hiver_data_rdbc::SqlxPoolClient> {
                &self.client
            }
        }
        ...
        impl hiver_data_commons::CrudRepository<#entity_type, #id_type> for #struct_name {
            type Error = hiver_data_commons::Error;

            async fn save(&self, entity: #entity_type) -> Result<#entity_type, Self::Error> {
                let pk = entity.primary_key().map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                let sql = format!(
                    "INSERT INTO {} (id) VALUES ($1) ON CONFLICT (id) DO UPDATE SET id = EXCLUDED.id",
                    self.table
                );
                self.client.execute_params(&sql, &[hiver_data_rdbc::QueryParam::Text(pk)]).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                Ok(entity)
            }
            ...
            async fn find_by_id(&self, id: #id_type) -> Result<Option<#entity_type>, Self::Error> {
                ...
                let id_param = hiver_data_rdbc::QueryParam::Text(id.to_string());
                match self.client.fetch_one_params(&sql, &[id_param]).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))? {
                    Some(row) => row.deserialize().map(Some).map_err(|e| hiver_data_commons::Error::other(e.to_string())),
                    None => Ok(None),
                }
            }
            ...
            async fn find_all(&self) -> Result<Vec<#entity_type>, Self::Error> {
                ...
                let rows = self.client.fetch_all(&sql).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                let mut results = Vec::with_capacity(rows.len());
                for row in &rows {
                    results.push(row.deserialize().map_err(|e| hiver_data_commons::Error::other(e.to_string()))?);
                }
                Ok(results)
            }
            ...
            async fn delete_by_id(&self, id: #id_type) -> Result<bool, Self::Error> {
                ...
                let id_param = hiver_data_rdbc::QueryParam::Text(id.to_string());
                let affected = self.client.execute_params(&sql, &[id_param]).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                Ok(affected > 0)
            }
            ...
            async fn delete(&self, entity: #entity_type) -> Result<bool, Self::Error> {
                let pk = entity.primary_key().map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                let sql = format!("DELETE FROM {} WHERE id = $1", self.table);
                let affected = self.client.execute_params(&sql, &[hiver_data_rdbc::QueryParam::Text(pk)]).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
                Ok(affected > 0)
            }

            async fn delete_all(&self) -> Result<usize, Self::Error> {
                let sql = format!("DELETE FROM {}", self.table);
                let affected = self.client.execute_cmd(&sql).await.map_err(|e| hiver_data_commons::Error::other(e.to_string()))?;
        ...
        impl #struct_name {
            pub fn with_url(url: &str) -> Result<Self, Box<dyn std::error::Error>> {
                use hiver_data_rdbc::{DatabaseConfig, SqlxPoolClient};
...
fn generate_query_method(
    ...
    let params_build = quote! {
        let params: Vec<hiver_data_rdbc::QueryParam> = vec![
            #(hiver_data_rdbc::QueryParam::from(#args)),*
        ];
    };
