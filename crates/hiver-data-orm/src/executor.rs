...
use crate::mapper::{BeanRowMapper, FirstRowExtractor, MappingResultSetExtractor, ResultSetExtractor};
...
impl<E: DatabaseClient> QueryExecutor<E> {
    ...
    pub async fn fetch_one_with_mapper<M: crate::mapper::RowMapper<T>, T>(
        &self, meta: &QueryMetadata, params: &HashMap<String, serde_json::Value>, mapper: &M,
    ) -> R2dbcResult<Option<T>> {
    ...
    pub async fn fetch_with_extractor<X: ResultSetExtractor<T>, T>(
        &self, meta: &QueryMetadata, params: &HashMap<String, serde_json::Value>, extractor: &X,
    ) -> R2dbcResult<T> {
        let (sql, vals) = meta.bind_params(params)?;
        let rows = self.client.fetch_all_params(&sql, &vals).await?;
...
mod tests {
    ...
    #[test] fn test_type_alias() {
        fn _check<E: DatabaseClient>(c: E) -> AnnotatedQueryExecutor<E> { QueryExecutor::new(c) }
    }
}
