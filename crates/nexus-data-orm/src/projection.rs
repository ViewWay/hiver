...
//!
//! ```rust,no_run,ignore
//! use nexus_data_orm::projection::{Projection, DtoProjection};
//!
...
pub trait Projection: Send + Sync {
    ...
    /// 将数据库行映射到投影类型。
    fn from_row(row: &nexus_data_rdbc::Row) -> crate::Result<Self::Target>;
}
...
///
/// ```rust,no_run,ignore
/// use nexus_data_orm::projection::InterfaceProjection;
///
...
///
/// ```rust,no_run,ignore
/// use nexus_data_orm::projection::DtoProjection;
///
/// #[derive(Deserialize)]
/// struct UserSummary {
///     name: String,
    ...
    T: serde::de::DeserializeOwned + Send + Sync,
{
    ...
    pub async fn query_where<C: DatabaseClient>(
        ...
        client: &C,
        condition: &str,
        params: &[nexus_data_rdbc::QueryParam],
    ) -> crate::Result<Vec<T>> {
        let col_list = if self.columns.is_empty() {
            "*".to_string()
        } else {
...
///
/// ```rust,no_run,ignore
/// use nexus_data_orm::projection::ClosedProjection;
///
...
impl<M: Model> DynamicProjection<M> {
    ...
    pub async fn execute<C: DatabaseClient>(
        &self,
        client: &C,
    ) -> crate::Result<Vec<nexus_data_rdbc::Row>> {
        let col_list = if self.columns.is_empty() {
            "*".to_string()
        } else {
            self.columns.join(", ")
