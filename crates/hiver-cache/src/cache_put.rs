...
///
/// ```rust,no_run,ignore
/// use hiver_cache::cache_put::CachePut;
///
/// #[derive(Clone)]
/// struct UserService {
///     cache: Arc<StringCache<User>>,
...
impl CachePutExec
{
    ...
    pub async fn execute_and_update<K, V, Fut, F>(cache: &dyn Cache<K, V>, key: K, f: F) -> V
    where
    ...
    pub async fn execute_and_update_with_ttl<K, V, Fut, F>(
        cache: &dyn Cache<K, V>,
        key: K,
        ttl_secs: u64,
        f: F,
    ...
    /// 执行函数并将结果放入缓存，带错误处理
    pub async fn execute_and_update_result<K, V, E, Fut, F>(
        cache: &dyn Cache<K, V>,
        key: K,
        f: F,
    ) -> Result<V, E>
