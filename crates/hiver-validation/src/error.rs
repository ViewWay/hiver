...
//!
//! ```rust,ignore
//! use hiver_validation::{ValidationError, ValidationErrors};
//!
//! // 单个字段错误
...
pub struct ValidationError
{
    ...
    /// 拒绝的值（被验证拒绝的值的字符串表示）/ Rejected value (string repr of the value that
    /// failed)
    pub rejected_value: Option<String>,
    /// 约束名称（导致失败的验证约束名）/ Constraint name (the validation constraint that caused
    /// the failure)
