...
//!
//! ```rust,no_run,ignore
//! use hiver_http::MultipartFile;
//! use hiver_http::multipart::MultipartForm;
//!
//! #[derive(Deserialize)]
...
///
/// ```rust,no_run,ignore
/// use hiver_http::multipart::MultipartData;
///
/// #[post("/upload")]
...
impl<T: for<'de> Deserialize<'de> + Send> FromMultipart for MultipartForm<T>
{
    fn from_multipart(req: &Request) -> Pin<Box<dyn Future<Output = Result<Self>> + Send + '_>>
    {
        Box::pin(async move {
            ...
            // For now, try to deserialize as JSON (form-data deserialization would be implemented
            // here) 现在尝试作为 JSON 反序列化（form-data 反序列化将在这里实现）
