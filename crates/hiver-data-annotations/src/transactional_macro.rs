...
/// # Attributes / 属性
///
/// - `isolation` - Transaction isolation level (default: Default) 事务隔离级别（默认：Default）
/// - `timeout` - Transaction timeout in seconds (default: 30) 事务超时时间（秒，默认：30）
/// - `propagation` - Transaction propagation behavior (default: Required)
///   事务传播行为（默认：Required）
/// - `read_only` - Whether transaction is read-only (default: false) 事务是否只读（默认：false）
/// - `max_retries` - Max retry attempts (default: 3) 最大重试次数（默认：3）
///
