    ...
    let register_border =
        |style: &ExcelCellStyle, borders_section: &mut String, next_border_id: &mut u32| {
            if style.border {
                ...
                let _ = write!(
                    borders_section,
                    "<border><left style=\"thin\"><color auto=\"1\"/></left><right \
                     style=\"thin\"><color auto=\"1\"/></right><top style=\"thin\"><color \
                     auto=\"1\"/></top><bottom style=\"thin\"><color \
                     auto=\"1\"/></bottom><diagonal/></border>"
                );
                *next_border_id += 1;
                Some(id)
            } else {
...
"#,
    ...
    if exporter.config.freeze_header && has_header {
        ...
        let _ = writeln!(
            xml,
            "<sheetViews><sheetView tabSelected=\"1\" workbookViewId=\"0\"><pane ySplit=\"1\" \
