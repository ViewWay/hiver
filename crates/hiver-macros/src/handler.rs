...
pub(crate) fn handler_impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let extract_statements = param_info.iter().map(|(pat, ty)| {
        quote! {
            let #pat: #ty = match <#ty as hiver_http::FromRequest>::from_request(&req).await {
                Ok(val) => val,
                Err(e) => {
                    return hiver_http::IntoResponse::into_response(
                        hiver_http::Response::builder()
                            .status(hiver_http::StatusCode::BAD_REQUEST)
                            .body(format!("Parameter extraction failed: {:?}", e))
                            .unwrap_or_else(|_| hiver_http::Response::new(hiver_http::Body::empty()))
                    );
                }
            };
        }
    ...
    let expanded = quote! {
        ...
        ::hiver_http::handler::register_handler(
            stringify!(#fn_name),
            |req: hiver_http::Request| async move {
                ...
                hiver_http::IntoResponse::into_response(result)
            }
        );
    };
