...
//!
//! ```rust,no_run,ignore
//! use nexus_test::TestApplication;
//!
//! #[tokio::test]
...
use crate::test_context::TestApplicationContext;
use std::any::Any;
...
impl TestApplicationBuilder {
    ...
    /// Register a bean factory function that will be invoked during [`start()`](TestApplication::start).
    /// 注册一个bean工厂函数，将在 [`start()`](TestApplication::start) 期间调用。
