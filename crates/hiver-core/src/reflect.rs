...
impl ReflectContainer {
        ...
        F: ReflectBeanFactory + 'static,
    {
        ...
        let mut factories_by_name = self.factories_by_name.write().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn create_bean_by_name(&self, type_name: &str) -> Result<Box<dyn Reflect>> {
        let factories = self.factories_by_name.read().unwrap_or_else(std::sync::PoisonError::into_inner);
