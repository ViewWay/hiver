//! Integration tests for nexus-data-orm
//! nexus-data-orm 的集成测试
...
mod tests {
    ...
    impl nexus_data_rdbc::DatabaseClient for MockDbClient {
        async fn fetch_all(&self, sql: &str) -> nexus_data_rdbc::Result<Vec<nexus_data_rdbc::Row>> {
            let conn = self.conn.lock().unwrap();
            let results = conn.query(sql).map_err(|e| nexus_data_rdbc::Error::Sql(e.to_string()))?;
            let rows: Vec<nexus_data_rdbc::Row> = results.into_iter().map(|map| {
                let mut row = nexus_data_rdbc::Row::new();
                for (k, v) in map {
                    let cv = match v {
                        serde_json::Value::Number(n) => {
                            if let Some(i) = n.as_i64() {
                                nexus_data_rdbc::ColumnValue::I64(i)
                            } else if let Some(f) = n.as_f64() {
                                nexus_data_rdbc::ColumnValue::F64(f)
                            } else {
                                nexus_data_rdbc::ColumnValue::Null
                            }
                        }
                        serde_json::Value::String(s) => nexus_data_rdbc::ColumnValue::String(s),
                        serde_json::Value::Bool(b) => nexus_data_rdbc::ColumnValue::Bool(b),
                        _ => nexus_data_rdbc::ColumnValue::Null,
                    };
        ...
        async fn fetch_one(&self, sql: &str) -> nexus_data_rdbc::Result<Option<nexus_data_rdbc::Row>> {
            let rows = self.fetch_all(sql).await?;
            Ok(rows.into_iter().next())
        }

        async fn execute_cmd(&self, sql: &str) -> nexus_data_rdbc::Result<u64> {
            let conn = self.conn.lock().unwrap();
            conn.execute(sql).map_err(|e| nexus_data_rdbc::Error::Sql(e.to_string()))?;
            Ok(1)
        }

        async fn begin_transaction(&self) -> nexus_data_rdbc::Result<nexus_data_rdbc::Transaction> {
            Err(nexus_data_rdbc::Error::Sql("transactions not supported in mock".into()))
        }

        async fn ping(&self) -> nexus_data_rdbc::Result<()> {
            Ok(())
        }

        async fn close(&self) -> nexus_data_rdbc::Result<()> {
            Ok(())
        }
    }
    ...
    #[test]
    fn smoke_test() {
        assert!(true, "nexus-data-orm test infrastructure is working");
    }
    // ── Edge case tests ──
