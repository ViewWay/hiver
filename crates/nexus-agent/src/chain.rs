...
use nexus_ai::chat_model::{ChatModel, ChatRequest};
...
///
/// ```rust,ignore
/// use nexus_agent::chain::AgentChain;
///
...
///
/// ```rust,ignore
/// use nexus_agent::chain::MapReduceAgent;
///
...
///
/// ```rust,ignore
/// use nexus_agent::chain::RouterAgent;
///
...
impl RouterAgent {
    ...
    async fn classify(&self, input: &str) -> Result<String, AgentError> {
        ...
        let request = ChatRequest::new()
            .message(nexus_ai::chat_model::ChatMessage::user(&prompt))
            .temperature(0.0)
            .max_tokens(50);
...
mod tests {
    ...
    fn test_router_agent_new() {
        use nexus_ai::chat_model::ModelError;
        struct MockModel;
        #[async_trait::async_trait]
        impl ChatModel for MockModel {
            async fn complete(&self, _request: ChatRequest) -> Result<nexus_ai::chat_model::ChatResponse, ModelError> {
                Ok(nexus_ai::chat_model::ChatResponse::new("general", "mock"))
            }
            async fn stream(&self, _request: ChatRequest) -> Result<nexus_ai::chat_model::ChatStream, ModelError> {
