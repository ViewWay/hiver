...
pub fn configuration_properties(attr: TokenStream, item: TokenStream) -> TokenStream
{
    ...
    let expanded = quote! {
        ...
        impl #name {
            ...
            pub fn from_context(ctx: &::hiver_starter::core::ApplicationContext) -> Self {
...
pub fn feign_client(attr: TokenStream, item: TokenStream) -> TokenStream
{
    ...
    {
        let TraitItem::Fn(method) = trait_item
        else
        {
            continue;
        };
        ...
        let Some(http_meth) = http_meth_tok
        else
        {
            continue;
        };
        ...
        let body = if let Some(bp) = &body_param
        {
            quote! {
                ...
                #query_code
                ::hiver_cloud::feign::execute_request_with_body(&self.client, #http_meth, &url, &#bp, vec![]).await
            }
        }
        else
        {
            quote! {
                let url = format!(concat!("{}", #url_fmt), self.base_url #(, #url_args)*);
                #query_code
                ::hiver_cloud::feign::execute_request(&self.client, #http_meth, &url, vec![]).await
            }
        };
    ...
    quote! {
        ...
        impl #client_name {
            ...
            /// Create from [`::hiver_cloud::feign::FeignClientConfig`].
            pub fn from_config(cfg: ::hiver_cloud::feign::FeignClientConfig) -> Result<Self, ::reqwest::Error> {
